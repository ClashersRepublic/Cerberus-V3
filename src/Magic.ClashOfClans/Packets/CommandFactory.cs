...
using Magic.PacketProcessing.Commands;
...
namespace Magic.PacketProcessing
{
    internal static class CommandFactory
    {
        ...
        public static object Read(PacketReader reader, int depth)
        {
            var cm = reader.ReadUInt32();
            if (m_vCommands.ContainsKey(cm))
            {
                var instance = (Command)Activator.CreateInstance(m_vCommands[cm], reader);
