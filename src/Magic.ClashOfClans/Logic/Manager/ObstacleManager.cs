    ...
    internal class ObstacleManager
    {
        ...
        public ObstacleManager(Level level)
        {
            ...
            if (m_vObstacleLimit == -1)
            {
                ...
                m_vObstacleRespawnSeconds = CsvManager.DataTables.GetGlobals().GetGlobalData("OBSTACLE_RESPAWN_SECONDS").NumberValue;
            }
            ...
            m_vNormalTimer.StartTimer(m_vObstacleRespawnSeconds, level.Time);
            m_vGemBoxTimer.StartTimer(m_vObstacleRespawnSeconds * 2, level.Time);
            m_vSpecialTimer.StartTimer(m_vObstacleRespawnSeconds, level.Time);
        ...
        public void Load(JObject jsonObject)
        {
            ...
            {
                m_vNormalTimer.StartTimer(m_vObstacleRespawnSeconds, m_vLevel.Time);
                m_vGemBoxTimer.StartTimer(m_vObstacleRespawnSeconds * 2, m_vLevel.Time);
                m_vSpecialTimer.StartTimer(m_vObstacleRespawnSeconds, m_vLevel.Time);
            }
        }
        ...
        public JObject Save(JObject jsonData)
        {
            ...
            if (m_vNormalTimer != null)
            {
                jobj.Add("secondsFromLastRespawn",
                    m_vObstacleRespawnSeconds - m_vNormalTimer.GetRemainingSeconds(m_vLevel.Time));
                jobj.Add("time_to_gembox_drop", m_vGemBoxTimer.GetRemainingSeconds(m_vLevel.Time));
                jobj.Add("time_to_special_drop", m_vSpecialTimer.GetRemainingSeconds(m_vLevel.Time));
        ...
        public void Tick()
        {
            while (m_vObstacleClearCount > 0 && m_vNormalTimer.GetRemainingSeconds(m_vLevel.Time) <= 0)
            {
            ...
            if (m_vGemBoxTimer.GetRemainingSeconds(m_vLevel.Time) <= 0)
            {
            ...
            try
            {
                ...
                foreach (var list in m_vLevel.GameObjectManager.GetAllGameObjects())
                {
                    foreach (var go in list)
                    {
                        ...
                        switch (go.Data.DataType)
                        {
                            case 0:
                                x--;
                                y--;
                                w = ((BuildingData)go.Data).Width + 2;
                                h = ((BuildingData)go.Data).Height + 2;
                                break;

                            case 7:
                                w = ((ObstacleData)go.Data).Width;
                                h = ((ObstacleData)go.Data).Height;
                                break;

                            case 11:
                                x--;
                                y--;
                                w = ((TrapData)go.Data).Width + 2;
                                h = ((TrapData)go.Data).Height + 2;
                                break;

                            case 17:
                                x--;
                                y--;
                                w = ((DecoData)go.Data).Width + 2;
                                h = ((DecoData)go.Data).Height + 2;
                                break;
                        }
        ...
        private void SpawnObstacle(int[] position, ObstacleData data)
        {
            ...
            o.SetPosition(position[0], position[1], m_vLevel.Avatar.GetActiveLayout());
