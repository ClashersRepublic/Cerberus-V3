...
namespace Magic.ClashOfClans.Network.Commands.Client
{
    internal class KickAllianceMemberCommand : Command
    {
        ...
        {
            var targetAccount = ResourcesManagerOld.GetPlayer(m_vAvatarId);
            if (targetAccount != null)
            {
                ...
                if (requesterAllianceId > 0 && targetAllianceId == requesterAllianceId)
                {
                    ...
                    if (targetMember.HasLowerRoleThan(requesterMember.GetRole()))
                    {
                        ...
                        if (ResourcesManagerOld.IsPlayerOnline(targetAccount))
                        {
                        ...
                        {
                            var alliancemembers = ResourcesManagerOld.GetPlayer(op.GetAvatarId());
