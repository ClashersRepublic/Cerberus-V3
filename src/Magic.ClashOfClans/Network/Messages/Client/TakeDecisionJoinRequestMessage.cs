...
namespace Magic.ClashOfClans.Network.Messages.Client
{
    ...
    internal class TakeDecisionJoinRequestMessage : Message
    {
        ...
        public override void Process(Level level)
        {
            ...
            Level requester = ResourcesManagerOld.GetPlayer(message.GetSenderId());
            if (Choice == 1)
            {
                if (!a.IsAllianceFull)
                {
                    ...
                    {
                        Level player = ResourcesManagerOld.GetPlayer(op.GetAvatarId());
                    ...
                    if (ResourcesManagerOld.IsPlayerOnline(requester))
                    {
