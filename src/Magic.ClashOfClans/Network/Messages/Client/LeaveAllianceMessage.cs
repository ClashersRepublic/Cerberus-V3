        ...
        public override void Process(Level level)
        {
            ...
            if (avatar.GetAllianceRole() == 2 && alliance.AllianceMembers.Count > 1)
            {
                ...
                foreach (AllianceMemberEntry player in members.Where(player => player.GetRole() >= 3))
                {
                    ...
                    if (ResourcesManagerOld.IsPlayerOnline(ResourcesManagerOld.GetPlayer(player.GetAvatarId())))
                    {
                        ...
                        var d = new AvailableServerCommandMessage(ResourcesManagerOld.GetPlayer(player.GetAvatarId()).Client);
                        d.SetCommandId(8);
                        d.SetCommand(c);
                        d.Send();
                    }
                    ...
                    foreach (AllianceMemberEntry player in members)
                    {
                        ...
                        if (loop == id)
                        {
                            ...
                            if (ResourcesManagerOld.IsPlayerOnline(ResourcesManagerOld.GetPlayer(player.GetAvatarId())))
                            {
                                ...
                                var f = new AvailableServerCommandMessage(ResourcesManagerOld.GetPlayer(player.GetAvatarId()).Client);
                                f.SetCommandId(8);
                                f.SetCommand(e);
                                f.Send();
                            }
            ...
            if (alliance.AllianceMembers.Count > 0)
            {
                ...
                foreach (Level onlinePlayer in ResourcesManagerOld.OnlinePlayers)
                {
