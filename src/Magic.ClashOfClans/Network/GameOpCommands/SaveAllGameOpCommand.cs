    ...
    internal class SaveAllGameOpCommand : GameOpCommand
    {
        ...
        public override void Execute(Level level)
        {
            if (level.AccountPrivileges>= RequiredPrivileges)
            {
                ...
                pm.Send();
                var levels = DatabaseManager.Instance.Save(ResourcesManagerOld.GetInMemoryLevels());
                levels.Wait();
                ...
                pmm.Send();
                /* Confirmation */
                var clans = DatabaseManager.Instance.Save(ResourcesManagerOld.GetInMemoryAlliances());
                clans.Wait();
