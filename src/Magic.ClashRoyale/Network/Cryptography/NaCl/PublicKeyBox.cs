...
namespace Magic.Network.Cryptography.NaCl
{
    ...
    public static class PublicKeyBox
    {
        ...
        {
            var paddedCipher = new byte[message.Length + curve25519xsalsa20poly1305.crypto_secretbox_ZEROBYTES];
            ...
            if (curve25519xsalsa20poly1305.crypto_box(paddedCipher, paddedMessage, nonce, publicKey, secretKey) != 0)
                throw new CryptographicException("Failed to box PublicKeyBox.");

            var chiperLen = paddedCipher.Length - curve25519xsalsa20poly1305.crypto_secretbox_BOXZEROBYTES;
            var chiper = new byte[chiperLen];
            Buffer.BlockCopy(paddedCipher, curve25519xsalsa20poly1305.crypto_secretbox_BOXZEROBYTES, chiper, 0, chiperLen);
            return chiper;
        }

        public static byte[] Open(byte[] chiper, byte[] publicKey, byte[] secretKey, byte[] nonce)
        {
            var paddedCipher = new byte[chiper.Length + curve25519xsalsa20poly1305.crypto_secretbox_BOXZEROBYTES];
            var paddedMessage = new byte[chiper.Length + curve25519xsalsa20poly1305.crypto_secretbox_BOXZEROBYTES];
            Buffer.BlockCopy(chiper, 0, paddedCipher, curve25519xsalsa20poly1305.crypto_secretbox_BOXZEROBYTES, chiper.Length);

            if (curve25519xsalsa20poly1305.crypto_box_open(paddedMessage, paddedCipher, nonce, publicKey, secretKey) != 0)
