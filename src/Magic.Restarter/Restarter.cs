...
namespace Magic.Restarter
{
    ...
    public class Restarter
    {
            ...
            try
            {
                while (true)
                {
                    try
                    {
                        ...
                        var title = "UCS Restarter v" + Assembly.GetExecutingAssembly().GetName().Version + " - Remaining: " + remaining + ", Count: " + RestartedTimes.Count + ", Average Run Time: " + avgRunTime;
                        Console.Title = title;
                        ...
                        if (hasCrashed)
                        {
                            ...
                            ConsoleUtils.WriteLineCenterRed("Detected that " + ConfigurationManager.AppSettings["Path2UCS"] + " has been crashed.");
                            ConsoleUtils.WriteLineCenterYellow("-> Restarting " + ConfigurationManager.AppSettings["Path2UCS"] + " at " + DateTime.Now);
                        ...
                        {
                            ConsoleUtils.WriteLineCenterRed("||");
                            ConsoleUtils.WriteLineCenterRed("Detected that " + ConfigurationManager.AppSettings["Path2UCS"] + " has been closed.");
                            ConsoleUtils.WriteLineCenterYellow("-> Restarting " + ConfigurationManager.AppSettings["Path2UCS"] + " at " + DateTime.Now);
        ...
        private void Restart()
        {
            ...
            NextRestart = DateTime.Now.Add(RestartInterval);
            ConsoleUtils.WriteLineCenterGreen("||");
            ConsoleUtils.WriteLineCenterGreen("" + ConfigurationManager.AppSettings["Path2UCS"] + " has been restarted successfully at " + DateTime.Now);
        }
    }
}
