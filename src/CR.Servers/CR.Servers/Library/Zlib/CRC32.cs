...
namespace CR.Servers.Library.ZLib
{
    ...
    public class CRC32
    {
        ...
        /// compatibility with BZip2, you should pass true here. In the CRC-32 used by GZIP and
        /// PKZIP, the bits are not reversed; Therefore if you want a CRC32 with compatibility with
        ...
        /// <summary>
        /// Create an instance of the CRC32 class, specifying the polynomial and whether to reverse
        /// data bits or not.
        /// </summary>
        /// <param name="polynomial">
        /// The polynomial to use for the CRC, expressed in the reversed (LSB)
        /// format: the highest ordered bit in the polynomial value is the coefficient of the 0th
        ///         power; the second-highest order bit is the coefficient of the 1 power, and so on.
        ///         Expressed this way, the polynomial for the CRC-32C used in IEEE 802.3, is 0xEDB88320.
        /// </param>
        /// <param name="reverseBits">specify true if the instance should reverse data bits.</param>
        /// <remarks>
        /// <para>
        /// In the CRC-32 used by BZip2, the bits are reversed. Therefore if you want a CRC32 with
        /// compatibility with BZip2, you should pass true here for the <c>reverseBits</c> parameter.
        /// In the CRC-32 used by GZIP and PKZIP, the bits are not reversed; Therefore if you want a
        /// CRC32 with compatibility with those, you should pass false for the <c>reverseBits</c> parameter.
        /// </para>
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        /// This is useful when using a divide-and-conquer approach to calculating a CRC. Multiple
        /// threads can each calculate a CRC32 on a segment of the data, and then combine the
        ...
        /// <summary>
        /// Get the CRC32 for the given (word,byte) combo. This is a computation defined by PKzip for
        /// PKZIP 2.0 (weak) encryption.
        /// </summary>
        ...
        /// <remarks>
        /// <para>
        /// Use this when employing a single instance of this class to compute multiple, distinct
        /// CRCs on multiple, distinct data blocks.
        /// </para>
        /// </remarks>
        public void Reset()
        {
        ...
        /// <summary>
        /// Update the value for the running CRC32 using the given block of bytes. This is useful
        /// when using the CRC32() class in a Stream.
        /// </summary>
        ...
        /// <remarks>
        /// <para>
        /// This method serves as an optimization for updating the CRC when a run of identical bytes
        /// is found. Rather than passing in a buffer of length n, containing all identical bytes b,
        /// this method accepts the byte value and the length of the (virtual) buffer - the length of
        /// the run.
        /// </para>
        /// </remarks>
        /// <param name="b">the byte to include into the CRC.</param>
    ...
    /// <remarks>
    /// <para>
    /// This class can be used to verify the CRC of a ZipEntry when reading from a stream, or to
    /// calculate a CRC when writing to a stream. The stream should be used to either read, or write,
    /// but not both. If you intermix reads and writes, the results are not defined.
    /// </para>
    ...
    public class CrcCalculatorStream : Stream, IDisposable
    {
        ...
        /// <remarks>
        /// <para>
        /// Instances returned from this constructor will leave the underlying stream open upon
        /// Close(). The stream uses the default CRC32 algorithm, which implies a polynomial of 0xEDB88320.
        /// </para>
        /// </remarks>
        ...
        /// <summary>
        /// A constructor allowing the specification of the length of the stream to read, as well as
        /// whether to keep the underlying stream open upon Close().
        /// </summary>
        /// <remarks>
        /// <para>
        /// The stream uses the default CRC32 algorithm, which implies a polynomial of 0xEDB88320.
        ...
        /// <summary>
        /// A constructor allowing the specification of the length of the stream to read, as well as
        /// whether to keep the underlying stream open upon Close(), and the CRC32 instance to use.
        /// </summary>
        /// <remarks>
        /// <para>
        /// The stream uses the specified CRC32 instance, which allows the application to specify how
        /// the CRC gets calculated.
        /// </para>
        /// </remarks>
        /// <param name="stream">The underlying stream</param>
        /// <param name="length">The length of the stream to slurp</param>
        ...
        // This ctor is private - no validation is done here. This is to allow the use of a
        ...
        /// <remarks>
        /// <para>
        /// The running total of the CRC is kept as data is written or read through the stream. read
        /// this property after all reads or writes to get an accurate CRC for the entire stream.
        /// </para>
        /// </remarks>
        ...
        /// <summary>
        /// The getter for this property returns the total bytes read. If you use the setter, it will
        ...
        /// </summary>
        /// <remarks>
        /// This is either the total number of bytes read, or the total number of bytes written,
        /// depending on the direction of this stream.
        /// </remarks>
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            // Need to limit the # of bytes returned, if the stream is intended to have a definite
            // length. This is especially useful when returning a stream for the uncompressed data
            // directly to the application. The app won't necessarily read only the UncompressedSize
            // number of bytes. For example wrapping the stream returned from OpenReader() into a
            // StreadReader() and calling ReadToEnd() on it, We can "over-read" the zip data and get
            // a corrupt string. The length limits that, prevents that problem.
            if (this._lengthLimit != CrcCalculatorStream.UnsetLengthLimit)
            {
