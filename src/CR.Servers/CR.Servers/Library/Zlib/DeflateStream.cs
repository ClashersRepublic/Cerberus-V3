namespace CR.Servers.Library.ZLib
{
    ...
    /// <remarks>
    /// <para>
    /// The DeflateStream is a <see href="http://en.wikipedia.org/wiki/Decorator_pattern"> Decorator
    /// </see> on a <see cref="System.IO.Stream"/> . It adds DEFLATE compression or decompression to
    /// any stream.
    /// </para>
    /// <para>
    ...
    /// reading or writing. The compression format used is DEFLATE, which is documented in <see
    /// href="http://www.ietf.org/rfc/rfc1951.txt"> IETF RFC 1951 </see> , "DEFLATE Compressed Data
    /// Format Specification version 1.3.".
    /// </para>
    /// <para>
    /// This class is similar to <see cref="ZlibStream"/>, except that <c>ZlibStream</c> adds the
    /// <see href="http://www.ietf.org/rfc/rfc1950.txt">RFC 1950 - ZLIB</see> framing bytes to a
    /// compressed stream when compressing, or expects the RFC1950 framing bytes when decompressing.
    /// The <c>DeflateStream</c> does not.
    /// </para>
    /// </remarks>
    ...
    public class DeflateStream : Stream
    {
        ...
        /// </summary>
        /// <remarks>
        /// When mode is <c>CompressionMode.Compress</c>, the DeflateStream will use the default
        /// compression level. The "captive" stream will be closed when the DeflateStream is closed.
        /// </remarks>
        /// <example>
        /// This example uses a DeflateStream to compress data from a file, and writes the compressed
        ...
        /// "captive" stream will be closed when the DeflateStream is closed.
        /// </para>
        /// </remarks>
        /// <example>
        /// This example uses a DeflateStream to compress data from a file, and writes the compressed
        ...
        /// <summary>
        /// Create a <c>DeflateStream</c> using the specified <c>CompressionMode</c>, and explicitly
        ...
        /// example if the stream is a memory stream that will be re-read after compression. Specify
        /// true for the <paramref name="leaveOpen"/> parameter to leave the stream open.
        /// </para>
        /// <para>The <c>DeflateStream</c> will use the default compression level.</para>
        ...
        /// </summary>
        /// <remarks>See the ZLIB documentation for the meaning of the flush behavior.</remarks>
        ...
        /// </summary>
        /// <remarks>Uncompress it with <see cref="DeflateStream.UncompressBuffer(byte[])"/>.</remarks>
        ...
        /// </summary>
        /// <remarks>Uncompress it with <see cref="DeflateStream.UncompressString(byte[])"/>.</remarks>
        ...
        /// <remarks>
        /// <para>
        /// If you wish to use the <c>DeflateStream</c> to compress data while reading, you can
        /// create a <c>DeflateStream</c> with <c>CompressionMode.Compress</c>, providing an
        /// uncompressed data stream. Then call Read() on that <c>DeflateStream</c>, and the data
        /// read will be compressed as you read. If you wish to use the <c>DeflateStream</c> to
        /// decompress data while reading, you can create a <c>DeflateStream</c> with
        /// <c>CompressionMode.Decompress</c>, providing a readable compressed data stream. Then call
        /// Read() on that <c>DeflateStream</c>, and the data read will be decompressed as you read.
        /// </para>
        /// <para>
        ...
        /// <remarks>
        /// <para>
        /// If you wish to use the <c>DeflateStream</c> to compress data while writing, you can
        /// create a <c>DeflateStream</c> with <c>CompressionMode.Compress</c>, and a writable output
        /// stream. Then call <c>Write()</c> on that <c>DeflateStream</c>, providing uncompressed
        /// data as input. The data sent to the output stream will be the compressed form of the data
        /// written. If you wish to use the <c>DeflateStream</c> to decompress data while writing,
        /// you can create a <c>DeflateStream</c> with <c>CompressionMode.Decompress</c>, and a
        /// writable output stream. Then call <c>Write()</c> on that stream, providing previously
        /// compressed data. The data sent to the output stream will be the decompressed form of the
        /// data written.
        /// </para>
        /// <para>
        /// A <c>DeflateStream</c> can be used for <c>Read()</c> or <c>Write()</c>, but not both.
        ...
        /// </para>
        /// <para>
        /// Application code won't call this code directly. This method may be invoked in two
        /// distinct scenarios. If disposing == true, the method has been called directly or
        /// indirectly by a user's code, for example via the public Dispose() method. In this case,
        /// both managed and unmanaged resources can be referenced and disposed. If disposing ==
        /// false, the method has been called by the runtime from inside the object finalizer and
        /// this method should not reference other objects; in that case only unmanaged resources
