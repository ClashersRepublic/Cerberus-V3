namespace CR.Servers.Library.ZLib
{
    ...
    internal sealed class DeflateManager
    {
        ...
        // always performed with a length multiple of the block size. To do: use the user input
        // buffer as sliding window.
        ...
        // file instead of a stored block (saving 5 bytes). This is applicable only for zip (not gzip
        // or zlib).
        ...
        // followed by a highly compressible string table.) Smaller buffer sizes give fast adaptation
        ...
        internal void pqdownheap(short[] tree, int k)
        {
            ...
            while (j <= this.heap_len)
            {
                ...
                if (j < this.heap_len && DeflateManager._IsSmaller(tree, this.heap[j + 1], this.heap[j], this.depth))
                {
                    j++;
                }

                // Exit if v is smaller than both sons
                if (DeflateManager._IsSmaller(tree, v, this.heap[j], this.depth))
                {
                    break;
                }
        ...
        private void _fillWindow()
        {
            ...
            do
            {
                ...
                n = this._codec.read_buf(this.window, this.strstart + this.lookahead, more);
                this.lookahead += n;
        ...
        internal BlockState DeflateSlow(FlushType flush)
        {
            ...
            while (true)
            {
                ...
                if (this.prev_length >= DeflateManager.MIN_MATCH && this.match_length <= this.prev_length)
                {
                    ...
                    bflush = this._tr_tally(this.strstart - 1 - this.prev_match, this.prev_length - DeflateManager.MIN_MATCH);
        ...
        internal int Initialize(ZlibCodec codec, CompressionLevel level, int windowBits, int memLevel, CompressionStrategy strategy)
        {
            ...
            this._lengthOffset = (1 + 2) * this.lit_bufsize;
        ...
        internal int SetParams(CompressionLevel level, CompressionStrategy strategy)
        {
            ...
            if (this.compressionLevel != level)
            {
                ...
                if (newConfig.Flavor != this.config.Flavor && this._codec.TotalBytesIn != 0)
                {
        ...
        internal int Deflate(FlushType flush)
        {
            ...
            if (this._codec.AvailableBytesOut == 0)
            {
                this._codec.Message = DeflateManager._ErrorMessage[ZlibConstants.Z_NEED_DICT - ZlibConstants.Z_BUF_ERROR];
            ...
            if (this.pendingCount != 0)
            {
                ...
                if (this._codec.AvailableBytesOut == 0)
                {
                    // System.out.println(" avail_out==0"); Since avail_out is 0, deflate will be
                    // called again with more output space, but possibly with both pending and
                    // avail_in equal to zero. There won't be anything to do, but this is not an
                    // error situation so make sure we return OK instead of BUF_ERROR at next call of deflate:
                    this.last_flush = -1;
                    return ZlibConstants.Z_OK;
                }
            ...
            else if (this._codec.AvailableBytesIn == 0 && (int) flush <= old_flush && flush != FlushType.Finish)
            {
                ...
                // FlushType.Finish, but... that seems very non-fatal. We can just say "OK" and do nothing.
