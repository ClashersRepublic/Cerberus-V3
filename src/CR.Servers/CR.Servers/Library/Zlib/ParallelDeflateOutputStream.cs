namespace CR.Servers.Library.ZLib
{
    ...
    /// </para>
    /// <para>
    /// This class is similar to <see cref="DeflateStream"/>, except that this class is for
    /// compression only, and this implementation uses an approach that employs multiple worker
    ...
    public class ParallelDeflateOutputStream : Stream
    {
        ...
        // TraceBits.Write | TraceBits.WriteBegin | TraceBits.WriteDone | TraceBits.Lifecycle |
        // TraceBits.Fill | TraceBits.Flush | TraceBits.Session;

        // private TraceBits _DesiredTrace = TraceBits.WriteBegin | TraceBits.WriteDone |
        // TraceBits.Synch | TraceBits.Lifecycle | TraceBits.Session ;
        private readonly TraceBits _DesiredTrace = TraceBits.Session | TraceBits.Compress | TraceBits.WriteTake | TraceBits.WriteEnter | TraceBits.EmitEnter | TraceBits.EmitDone | TraceBits.EmitLock | TraceBits.EmitSkip | TraceBits.EmitBegin;
        ...
        /// </para>
        /// <para>
        /// This class is similar to <see cref="DeflateStream"/>, except that this implementation
        /// uses an approach that employs multiple worker threads to perform the DEFLATE. On a
        /// multi-cpu or multi-core computer, the performance of this class can be significantly
        /// higher than the single-threaded DeflateStream, particularly for larger streams. How
        ...
        ///               property (which is 16), and it runs on a machine with 2 CPU cores. In that
        ///               case, DotNetZip will allocate 4 buffer pairs per CPU core, for a total of 8
        ///               pairs. The upper limit specified by this property has no effect.
        /// </para>
        /// <para>
        ...
        /// <see cref="DeflateStream"/> . On the other hand, using a BufferSize of 8k can result in a
        /// compressed data stream that is 5% larger than that delivered by the single-threaded
        /// <c>DeflateStream</c>. Excessively small buffer sizes can also cause the speed of the
        /// ParallelDeflateOutputStream to drop, because of larger thread scheduling overhead dealing
        /// with many many small buffers.
        /// </para>
        /// <para>
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            {
                // Want to do this on first Write, first session, and not in the constructor. We want
                // to allow MaxBufferPairs to change after construction, but before first Write.
            ...
            do
            {
                ...
                if (this._currentlyFilling >= 0)
                {
                    ...
                    this.TraceOutput(TraceBits.WriteTake, "Write    notake   wi({0}) lf({1})", ix, this._lastFilled);
                }
                else
                {
                    ...
                    {
                        // no available buffers, so... need to emit compressed buffers.
                        mustWait = true;
                        continue;
                    }
        ...
        private void EmitPendingBuffers(bool doAll, bool mustWait)
        {
            ...
            // stream, if this stream is employed within a using or try/finally pair as required. But
                    ...
                    if (Monitor.TryEnter(this._toWrite, millisecondsToWait))
                    {
                        ...
                        if (nextToWrite >= 0)
                        {
                            ...
                            if (workitem.ordinal != this._lastWritten + 1)
                            {
                                ...
                                {
                                    // We went around the list once. None of the items in the list is
                                    // the one we want. Now wait for a compressor to signal again.
                                    this._newlyCompressedBlob.WaitOne();
                                    firstSkip = -1;
                                }
                                else if (firstSkip == -1)
        ...
        private void _PerpetualWriterMethod(object state)
        {
            ...
            try
            {
                do
                {
                        ...
                        lock(workitem)
                        {
                            ...
                            do
                            {
                                ...
                                else
                                {
                                    ...
                                    // I've locked a workitem I cannot use. Therefore, wake someone
                                    // else up, and then release the lock.
                    ...
                    // Finish: After writing a series of buffers, closing each one with Flush.Sync,
                    // we now write the final one as Flush.Finish, and then stop.
                    byte[] buffer = new byte[128];
                    ZlibCodec compressor = new ZlibCodec();
